...
def find_classes(directory : str) -> Tuple[List[str], Dict[str,int]]:
    ...
    # 3. Create a directory of index labels (computers prefer numerical rather than string labels)
...
def display_random_images(dataset: torch.utils.data.dataset.Dataset,
    ...
    for i , targ_sample in enumerate(random_samples_idx):
        ...
        plt.title(title)
# %%
# Turn train and test custom Datset's into DataLoader's 
from torch.utils.data import DataLoader 
...
# %%
a = [[1,3,3],[3,5,6],[3,4,5],[4,4,4]]
arr = np.array(a)
test_acc = 0 
b = 3 
for i in arr:
    print((i==b).sum().item())
    print(f"sum type : {type((i==b).sum())}")
    print(f"item type: {type((i==b).sum().item())}")
    test_acc += ((i == b).sum().item()/len(a))
test_acc
# print(arr)
# %%
...
# %%
# Cehck the model_0_results keys 
model_0_results.keys()
model_0_results['train_loss']
model_0_results['train_acc']
len(model_0_results["train_loss"]),len(model_0_results['train_acc'])
# %%
...
# End the timer and print out how long it work 
end_time = timer()
print(f"Total training time: {end_time-start_time:.3f} seconds")
...
custom_image = totensor(Image.open(custom_image_path))

# Print out image data 
print(f"Custom image tensor: \n{custom_image}\n")
print(f"Custsom image shape: {custom_image.shape}\n")
...
def pred_and_plot_image(model: torch.nn.Module,
    ...
    totensor = torchvision.transforms.ToTensor()
    target_image = totensor(Image.open(str(image_path)))
